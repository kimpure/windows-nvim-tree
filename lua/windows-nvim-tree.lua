local config = require("windows-nvim-tree.config")
local fs = require("windows-nvim-tree.fs")

--- @class WindowsNvimTree
local M = {}

--- @param options? WindowsNvimTree.Config
function M.setup(options)
	options = vim.tbl_deep_extend("force", config.default, options or {})

	if not options.iswindows then
		return
	end
	...
	if not installed then
		return vim.health.error("nvim-tree is missing")
	end
	...
	local function on_attach(bufnr)
		...
		vim.keymap.set("n", "a", create, opts("Create File Or Directory"))
		vim.keymap.set("n", "bd", nvim_tree_api.marks.bulk.delete, opts("Delete Bookmarked"))
		vim.keymap.set("n", "bt", nvim_tree_api.marks.bulk.trash, opts("Trash Bookmarked"))
		vim.keymap.set("n", "bmv", nvim_tree_api.marks.bulk.move, opts("Move Bookmarked"))
		vim.keymap.set("n", "B", nvim_tree_api.tree.toggle_no_buffer_filter, opts("Toggle Filter: No Buffer"))
		vim.keymap.set("n", "c", nvim_tree_api.fs.copy.node, opts("Copy"))
		vim.keymap.set("n", "C", nvim_tree_api.tree.toggle_git_clean_filter, opts("Toggle Filter: Git Clean"))
		...
		vim.keymap.set("n", "F", nvim_tree_api.live_filter.clear, opts("Live Filter: Clear"))
		vim.keymap.set("n", "f", nvim_tree_api.live_filter.start, opts("Live Filter: Start"))
		...
		vim.keymap.set("n", "ge", nvim_tree_api.fs.copy.basename, opts("Copy Basename"))
		vim.keymap.set("n", "H", nvim_tree_api.tree.toggle_hidden_filter, opts("Toggle Filter: Dotfiles"))
		vim.keymap.set("n", "I", nvim_tree_api.tree.toggle_gitignore_filter, opts("Toggle Filter: Git Ignore"))
		...
		vim.keymap.set("n", "L", nvim_tree_api.node.open.toggle_group_empty, opts("Toggle Group Empty"))
		vim.keymap.set("n", "M", nvim_tree_api.tree.toggle_no_bookmark_filter, opts("Toggle Filter: No Bookmark"))
		...
		vim.keymap.set("n", "u", nvim_tree_api.fs.rename_full, opts("Rename: Full Path"))
		vim.keymap.set("n", "U", nvim_tree_api.tree.toggle_custom_filter, opts("Toggle Filter: Hidden"))
		vim.keymap.set("n", "W", nvim_tree_api.tree.collapse_all, opts("Collapse All"))
